...
class TadoAuthenticationClientIntegrationTestOld(
  ...
  /*
    "authenticates correctly" {
      val result = tadoAuthenticationClient.token(TadoAuthTokenRequest(configuration))
      ...
      mock.authServer.verify(
        1,
        postRequestedFor(urlEqualTo(TadoAuthenticationClient.TOKEN_PATH))
          ...
          .withFormParam(TadoAuthRequest.P_GRANT_TYPE, TadoAuthTokenRequest.GRANT_TYPE)
      )
  ...
  */
  "refresh with invalid refresh token fails" {
    ...
    shouldThrow<HttpClientErrorException> {
      tadoAuthenticationClient.token(TadoAuthRefreshRequest(configuration, REFRESH_TOKEN))
    }
*/
    mock.authServer.verify(
      1,
